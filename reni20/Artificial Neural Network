library("nnet")
fishing<- read.csv("E:\\KULIAH\\STATISTIKA 6\\DATMIN\\Fishing.csv")
head(fishing)
dim(fishing)
...
{
fishing1<-nnet(mode~., data=fishing.train, size=jumlahhidden,decay=5e-4, trace=F)
y<- fishing.test$mode
p<- predict(fishing1, fishing.test, type="class")
err<- mean(y!=p)
c(jumlahhidden, err)
...
plot(hasilakhir, type="b", xlab="jumlah hidden unit", ylab="error",main="plot jumlah hidden unit vs error")
